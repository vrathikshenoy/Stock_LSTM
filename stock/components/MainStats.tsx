...
const MainStats = ({ ticker, data, currencySymbol = "$" }) => {
  ...
  return (
    <motion.div variants={container} initial="hidden" animate="show" className="grid grid-cols-1 gap-6 w-full">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
          ...
          <div className="flex-1">
            <div className="flex items-center gap-3 mb-3">
              ...
              <span className="text-gray-500 text-sm bg-gray-100 px-2 py-1 rounded-md">{data.market_summary.date}</span>
            </div>
          ...
          <div className="flex-1 border-t md:border-t-0 md:border-l border-gray-200 pt-6 md:pt-0 md:pl-6">
            <h3 className="text-base font-medium text-gray-700 mb-4 flex items-center">
              <DollarSign className="h-5 w-5 mr-2 text-purple-600" /> Market Summary
            </h3>
            <div className="grid grid-cols-2 gap-x-8 gap-y-4">
              ...
              <div>
                ...
                <p className="text-lg font-semibold">{(data.market_summary.volume / 1000000).toFixed(2)}M</p>
              </div>
            </div>
          </div>
          ...
          <div className="flex-1 border-t md:border-t-0 md:border-l border-gray-200 pt-6 md:pt-0 md:pl-6">
            <h3 className="text-base font-medium text-gray-700 mb-4 flex items-center">
              <TrendingUp className="h-5 w-5 mr-2 text-purple-600" /> Prediction Info
            </h3>
            <div className="grid grid-cols-1 gap-y-4">
              <div className="flex justify-between items-center">
                ...
                <p
                  className={`text-lg font-semibold ${
                    data.future_predictions.values["10"][9] > data.latest_price ? "text-green-600" : "text-red-600"
                  }`}
                >
                  {currencySymbol}
              ...
              <div className="flex justify-between items-center">
                ...
                <p
                  className={`text-lg font-semibold ${
                    data.future_predictions.values["20"][19] > data.latest_price ? "text-green-600" : "text-red-600"
                  }`}
                >
                  {currencySymbol}
              ...
              <div className="flex justify-between items-center">
                ...
                <p
                  className={`text-lg font-semibold ${
                    data.future_predictions.values["50"][49] > data.latest_price ? "text-green-600" : "text-red-600"
                  }`}
                >
                  {currencySymbol}
              ...
              <div>
                <div className="flex justify-between items-center mb-1">
                  ...
                  <span className="font-semibold">
                    {Math.max(0, 100 - (data.rmse / data.latest_price) * 100).toFixed(1)}%
                  </span>
                </div>
                <div className="w-full bg-gray-200 rounded-full h-2">
                  <div
                    className={`h-2 rounded-full ${
                      Math.max(0, 100 - (data.rmse / data.latest_price) * 100) > 80
                        ? "bg-green-500"
                        : Math.max(0, 100 - (data.rmse / data.latest_price) * 100) > 60
                          ? "bg-yellow-500"
                          : "bg-red-500"
                    }`}
                    style={{
